...
describe('SearchProducts', () => {
  ...
  it('matches ignoring accents and case', async () => {
    const found = await useCase.execute(new SearchProductsInput('CAFE', null, false, false, false, false, 50, 0));
  ...
  it('matches partial multi-token queries', async () => {
    const found = await useCase.execute(new SearchProductsInput('inca 600', null, false, false, false, false, 50, 0));
  ...
  it('filters by category', async () => {
    const found = await useCase.execute(
      new SearchProductsInput(null, 'frutas-verduras', false, false, false, false, 50, 0),
    );
  ...
  it('excludes inactive products by default', async () => {
    const found = await useCase.execute(new SearchProductsInput('pilsen', null, false, false, false, false, 50, 0));

    expect(found.items).toHaveLength(0);
  });

  it('treats a blank query as no query filter', async () => {
    const found = await useCase.execute(new SearchProductsInput('   ', null, false, false, false, false, 50, 0));
